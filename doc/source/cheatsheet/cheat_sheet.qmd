...
ReadCaseData(settings_source=solver_session)(file_name=file_path)
```
...
graphics_window.add_graphics(xy_plane, position=(0, 0))

yz_plane = PlaneSurface.create_yz_plane(
    solver=solver_session, x=-0.174628)
graphics_window.add_graphics(yz_plane, position=(0, 1))

zx_plane = PlaneSurface.create_zx_plane(
    solver=solver_session, y=-0.0627297)
graphics_window.add_graphics(zx_plane, position=(0, 2))
...
xy_plane = PlaneSurface.create_from_point_and_normal(
    ...
    normal=[0.0, 0.0, 1.0])
graphics_window.add_graphics(xy_plane)
graphics_window.show()
```
...
mid_plane_x = IsoSurface(
    ...
    field="x-coordinate",
    iso_value=-0.174)
graphics_window.add_graphics(mid_plane_x)
graphics_window.show()
```
